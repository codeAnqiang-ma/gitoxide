...
mod tests {
    ...
    #[test]
    fn parses_git_compatible_gpg_status() {
        let outcome = parse_gpg_output(
            Format::OpenPgp,
            "Good signature".into(),
        ...
        assert_eq!(outcome.trust_level, TrustLevel::Fully);
        assert_eq!(
            outcome.signer.as_ref().map(|value| value.as_slice()),
            Some(b"Fixture Signer".as_slice())
        );
        assert_eq!(
            outcome.fingerprint.as_ref().map(|value| value.as_slice()),
            Some(b"FINGERPRINT".as_slice())
        );
        assert_eq!(
            outcome.primary_key_fingerprint.as_ref().map(|value| value.as_slice()),
